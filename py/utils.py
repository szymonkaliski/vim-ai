...
def parse_chat_messages(chat_content, role_prefix=None):
    ...
    for line in lines:
        match line:
            case _ if line.startswith(f"{role_prefix}>>> system"):
                messages.append({'role': 'system', 'content': [{'type': 'text', 'text': ''}]})
                current_type = 'system'
            case _ if line.startswith(f"{role_prefix}<<< thinking"):
                # nothing to do here, thinking messages are omitted
                current_type = 'thinking'
            case _ if line.startswith(f"{role_prefix}<<< assistant"):
                messages.append({'role': 'assistant', 'content': [{'type': 'text', 'text': ''}]})
                current_type = 'assistant'
            case _ if line.startswith(f"{role_prefix}>>> user"):
                if messages and messages[-1]['role'] == 'user':
                    messages[-1]['content'].append({'type': 'text', 'text': ''})
                else:
                    messages.append({'role': 'user', 'content': [{'type': 'text', 'text': ''}]})
                current_type = 'user'
            case _ if line.startswith(f"{role_prefix}>>> include"):
                if not messages or messages[-1]['role'] != 'user':
                    messages.append({'role': 'user', 'content': []})
                current_type = 'include'
            case _ if line.startswith(f"{role_prefix}>>> exec"):
                if not messages or messages[-1]['role'] != 'user':
                    messages.append({'role': 'user', 'content': []})
                current_type = 'exec'
            case _:
