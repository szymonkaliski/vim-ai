...
def run_ai_chat(context):
    ...
    def initialize_chat_window():
        ...
        contains_user_prompt = re.search(rf"^{re.escape(role_prefix)}>>> (user|exec|include)", file_content, flags=re.MULTILINE)
        ...
        if not contains_user_prompt:
            ...
            vim.command(f"normal! O{role_prefix}>>> user\n")
        ...
        last_role = re.search(rf"^{re.escape(role_prefix)}(>>>|<<<) (\w+)", file_content, flags=re.DOTALL | re.MULTILINE)
